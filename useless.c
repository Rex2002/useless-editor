...
#define USELESS_TABSTOP 8
#define USELESS_QUIT_TIMES 3
...
void enableRawMode(){
    ...
    // the & is not in the original, maybe later a source for headache
...
int editorRowCxToRx(erow *row, int cx){
    ...
    for(int j = 0; j < cx; j++){
        if(row->chars[j] == '\t'){
            rx += (USELESS_TABSTOP - 1) - (rx % USELESS_TABSTOP);
        }
        rx++;
    }
    return rx;
...
int editorRowRxToCx(erow *row, int rx){
    ...
    for(cx = 0; cx < row->size; cx++){
        if(row -> chars[cx] == '\t'){
            cur_rx += (USELESS_TABSTOP - 1) - (cur_rx % USELESS_TABSTOP);
        }
        cur_rx++;
...
void editorUpdateRow(erow *row){
    ...
    free(row->render);
    row->render = malloc(row->size+tabs*USELESS_TABSTOP+1);

    int idx = 0;
    for(j = 0; j < row->size; j++){
        if(row->chars[j] == '\t'){
            row->render[idx++] = ' ';
            while(idx % USELESS_TABSTOP != 0) row->render[idx++] = ' ';
        }
        else {
    ...
    for(y=0; y<E.screenrows; y++){
        ...
        if(filerow >= E.numrows) {
            if(E.numrows == 0 && y == E.screenrows / 3){
                ...
                "Useless Editor -- version %s", USELESS_VERSION);
...
void editorProcessKeypress(){
    static int quit_times = USELESS_QUIT_TIMES;
    int c = editorReadKey();
